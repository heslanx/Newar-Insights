...
func (h *HealthHandler) Health(c *fiber.Ctx) error {
	...
	if err := h.redis.HealthCheck(ctx); err != nil {
		...
		overallStatus = constants.HealthStatusDegraded
	} else {
		deps["redis"] = "ok"
	}
	...
	if overallStatus == constants.HealthStatusUnhealthy {
		statusCode = 503
	} else if overallStatus == constants.HealthStatusDegraded {
		statusCode = 200 // Still serve requests if only Redis is down
	}
