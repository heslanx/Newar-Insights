...
async def start_bot_container(
    ...
    environment = [
        ...
        f"RECORDINGS_DIR={os.getenv('RECORDINGS_DIR', '/recordings')}",
    ]
    ...
    create_payload = {
        ...
        "HostConfig": {
            ...
            "AutoRemove": True,  # AutoRemove is incompatible with RestartPolicy
            # *** SECURITY: Resource limits to prevent OOM and DoS ***
            "Memory": 1610612736,  # 1.5 GB in bytes (1536 MB)
            "MemorySwap": 1610612736,  # Same as Memory = no swap
            "CpuPeriod": 100000,  # CPU period in microseconds
            "CpuQuota": 100000,  # 1 CPU core (100% of 1 period)
            "OomKillDisable": False,  # Allow OOM killer if needed
            "Mounts": [
                {
                    "Type": "volume",
                    "Source": f"{os.getenv('COMPOSE_PROJECT_NAME', 'vexa_simple')}_recordings-data",
                    "Target": "/recordings",
                    "ReadOnly": False
                }
            ]
