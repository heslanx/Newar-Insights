...
from app.orchestrators import (
    ...
    stop_bot_container, get_running_bots_status,
)
...
async def schedule_status_webhook_task(
    ...
    logger.info(f"Meeting {meeting.id} status changed: {old_status} -> {new_status} (source: {transition_source})")
...
async def request_bot(
    ...
    try:
        ...
        # DISABLED: MeetingSession model does not exist in simplified recording system
        # asyncio.create_task(_record_session_start(meeting_id, connection_id))
        logger.info(f"Session {connection_id} started for meeting {meeting_id} (session tracking disabled)")
...
async def update_bot_config(
    ...
    # Use Redis mapping written at launch
    ...
    if not original_session_uid:
        logger.error(f"Active meeting {internal_meeting_id} found, but no associated session UID in Redis. Cannot send command.")
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
async def stop_bot(
    ...
    if not meeting.bot_container_id:
        ...
        logger.info(f"Post-meeting finalization completed for meeting {meeting.id} (no container case).")
    ...
    if meeting.status in [MeetingStatus.REQUESTED.value, MeetingStatus.JOINING.value, MeetingStatus.AWAITING_ADMISSION.value] and (seconds_since_created is not None and seconds_since_created < 5):
        ...
        if success:
            await publish_meeting_status_change(meeting.id, MeetingStatus.COMPLETED.value, redis_client, platform_value, native_meeting_id, meeting.user_id)
        # Post-meeting tasks completed
        logger.info(f"Post-meeting finalization completed for meeting {meeting.id} (pre-active).")
        return {"message": "Stop request accepted; meeting finalized immediately (pre-active)."}

    # 2. Find the session UID for this meeting from Redis
    earliest_session_uid: Optional[str] = None
    try:
        if redis_client:
            mapping_key = f"bm:meeting:{platform_value}:{native_meeting_id}:current_uid"
            cached_uid = await redis_client.get(mapping_key)
            if isinstance(cached_uid, str) and cached_uid:
                earliest_session_uid = cached_uid
                logger.info(f"Found session UID from Redis: {earliest_session_uid}")
    except Exception as e:
        logger.warning(f"Failed to read session UID from Redis: {e}")
    ...
    if not earliest_session_uid:
        ...
        logger.info(f"Found session UID '{earliest_session_uid}' for meeting {meeting.id}. Preparing to send leave command.")
...
async def bot_exit_callback(
    ...
    try:
        # Find the meeting by container_id
        container_id = payload.container_id
        if not container_id:
            logger.error(f"Bot exit callback: No container_id provided in payload")
            return {"status": "error", "detail": "container_id is required"}
        
        meeting_stmt = select(Meeting).where(Meeting.bot_container_id == container_id)
        meeting_result = await db.execute(meeting_stmt)
        meeting = meeting_result.scalars().first()

        if not meeting:
            logger.error(f"Bot exit callback: Could not find meeting for container_id {container_id}")
            return {"status": "error", "detail": f"Meeting not found for container_id: {container_id}"}

        logger.info(f"Bot exit callback: Found meeting_id {meeting.id} for container_id {container_id}")
        ...
        # ALWAYS finalize post-meeting tasks, regardless of exit code
        logger.info(f"Bot exit callback: Post-meeting finalization completed for meeting {meeting.id}.")
...
async def bot_startup_callback(
    ...
    try:
        # Find the meeting by container_id
        if not container_id:
            logger.error(f"Bot startup callback: No container_id provided in payload")
            return {"status": "error", "detail": "container_id is required"}
        ...
        if not meeting:
            logger.error(f"Bot startup callback: Could not find meeting for container_id {container_id}")
            return {"status": "error", "detail": f"Meeting not found for container_id: {container_id}"}

        logger.info(f"Bot startup callback: Found meeting_id {meeting.id} for container_id {container_id}")
        ...
        if meeting.data and isinstance(meeting.data, dict) and meeting.data.get("stop_requested"):
            logger.info(f"Bot startup callback: stop_requested set for meeting {meeting.id}. Ignoring startup transition.")
...
async def bot_joining_callback(
    ...
    try:
        # Find the meeting by container_id
        if not container_id:
            logger.error(f"Bot joining callback: No container_id provided in payload")
            raise HTTPException(
        ...
        if not meeting:
            logger.error(f"Bot joining callback: Could not find meeting for container_id {container_id}")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Meeting not found for container_id: {container_id}"
            )

        logger.info(f"Bot joining callback: Found meeting_id {meeting.id} for container_id {container_id}")
...
async def bot_awaiting_admission_callback(
    ...
    try:
        # Find the meeting by container_id
        if not container_id:
            logger.error(f"Bot awaiting admission callback: No container_id provided in payload")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="container_id is required"
            )
        ...
        if not meeting:
            logger.error(f"Bot awaiting admission callback: Could not find meeting for container_id {container_id}")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Meeting not found for container_id: {container_id}"
            )

        logger.info(f"Bot awaiting admission callback: Found meeting_id {meeting.id} for container_id {container_id}")
...
async def bot_status_change_callback(
    ...
    try:
        ...
        container_id = payload.container_id
        if not container_id:
            logger.error(f"Bot status change callback: No container_id provided in payload")
            return {"status": "error", "detail": "container_id is required"}
        ...
        if not meeting:
            logger.error(f"Bot status change callback: Could not find meeting for container_id {container_id}")
            return {"status": "error", "detail": f"Meeting not found for container_id: {container_id}"}

        logger.info(f"Bot status change callback: Found meeting_id {meeting.id} for container_id {container_id}")
        ...
        if new_status == MeetingStatus.COMPLETED:
            ...
            if success:
                ...
                # Post-meeting finalization completed
                logger.info(f"Post-meeting finalization completed for meeting {meeting.id}.")
