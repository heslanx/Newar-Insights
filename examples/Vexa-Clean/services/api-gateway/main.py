...
RECORDING_STORAGE_URL = os.getenv("RECORDING_STORAGE_URL")

# --- Validation at startup ---
if not all([ADMIN_API_URL, BOT_MANAGER_URL, RECORDING_STORAGE_URL]):
    missing_vars = [
        var_name
        for var_name, var_value in {
            "ADMIN_API_URL": ADMIN_API_URL,
            "BOT_MANAGER_URL": BOT_MANAGER_URL,
            "RECORDING_STORAGE_URL": RECORDING_STORAGE_URL,
        }.items()
        if not var_value
    ]
...
app = FastAPI(
    ...
    **Main entry point for the Vexa platform APIs.**
    ...
    Provides access to:
    - Bot Management (Starting/Stopping recording bots)
    - Recording Retrieval
    ...
    1.  **`X-API-Key`**: Required for all regular client operations (e.g., managing bots, getting recordings). Obtain your key from an administrator.
    ...
    """,
    version="2.0.0", # Incremented version for major breaking changes
    contact={
        "name": "Vexa Support",
...
@app.get("/recordings",
        tags=["Recordings"],
        summary="Get list of user's recordings",
        description="Returns a list of all recordings for the user associated with the API key.",
        dependencies=[Depends(api_key_scheme)])
async def get_recordings_proxy(request: Request):
    """Forward request to Recording Storage to get recordings."""
    url = f"{RECORDING_STORAGE_URL}/recordings"
    return await forward_request(app.state.http_client, "GET", url, request)

@app.get("/recordings/{platform}/{native_meeting_id}",
        tags=["Recordings"],
        summary="Download recording for a specific meeting",
        description="Downloads the audio recording file for a meeting specified by its platform and native ID.",
        dependencies=[Depends(api_key_scheme)])
async def download_recording_proxy(platform: Platform, native_meeting_id: str, request: Request):
    """Forward request to Recording Storage to download a recording."""
    url = f"{RECORDING_STORAGE_URL}/recordings/{platform.value}/{native_meeting_id}"
    return await forward_request(app.state.http_client, "GET", url, request)

@app.delete("/recordings/{platform}/{native_meeting_id}",
            tags=["Recordings"],
            summary="Delete recording for a specific meeting",
            description="Deletes the audio recording file for a meeting specified by its platform and native ID.",
            dependencies=[Depends(api_key_scheme)])
async def delete_recording_proxy(platform: Platform, native_meeting_id: str, request: Request):
    """Forward request to Recording Storage to delete a recording."""
    url = f"{RECORDING_STORAGE_URL}/recordings/{platform.value}/{native_meeting_id}"
    return await forward_request(app.state.http_client, "DELETE", url, request)
...
async def get_meetings_proxy(request: Request):
    """Forward request to Bot Manager to get meetings."""
    url = f"{BOT_MANAGER_URL}/meetings"
    return await forward_request(app.state.http_client, "GET", url, request)
...
# --- Removed transcription and WebSocket endpoints ---
# This simplified version focuses on recording management only
