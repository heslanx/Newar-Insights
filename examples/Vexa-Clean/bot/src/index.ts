...
let currentBotConfig: BotConfig | null = null; // ADDED: To store full bot config for graceful leave
  ...
  if (page && !page.isClosed()) { // Only attempt platform leave if page is valid
    try {
      ...
      if (currentPlatform === "google_meet") { // Add platform check if you have other platform handlers
         // FIXED: Pass botConfig and reason to enable recording extraction
         platformLeaveSuccess = await leaveGoogleMeet(page, currentBotConfig || undefined, reason);
      } else if (currentPlatform === "teams") {
         // FIXED: Pass botConfig and reason to enable recording extraction
         platformLeaveSuccess = await leaveMicrosoftTeams(page, currentBotConfig || undefined, reason);
      } else {
...
export async function runBot(botConfig: BotConfig): Promise<void> {
  ...
  currentBotConfig = botConfig; // ADDED: Store full bot config for graceful leave
