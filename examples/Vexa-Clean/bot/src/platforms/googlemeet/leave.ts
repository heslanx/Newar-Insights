...
import { extractRecording } from "./recording-mp3";
...
export async function leaveGoogleMeet(page: Page | null, botConfig?: BotConfig, reason: string = "manual_leave"): Promise<boolean> {
  ...
  // CRITICAL: Extract recording BEFORE leaving the meeting
  if (botConfig) {
    try {
      log("🎙️ Extracting recording before leaving meeting...");
      const recordingPath = await extractRecording(page, botConfig);
      if (recordingPath) {
        log(`✅ Recording extracted successfully: ${recordingPath}`);
      } else {
        log("⚠️ Warning: Recording extraction returned null (may be expected if no audio captured)");
      }
    } catch (extractError: any) {
      log(`⚠️ Warning: Failed to extract recording: ${extractError.message}`);
      log(`Stack: ${extractError.stack}`);
      // Continue with leave even if extraction fails
    }
  }

  // Call leave callback to notify bot-manager
  if (botConfig) {
    try {
